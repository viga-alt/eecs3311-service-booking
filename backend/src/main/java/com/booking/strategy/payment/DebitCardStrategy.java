...
/* STRATEGY PATTERN - Debit Card payment processing
 * Same validation rules as credit card (16-digit number, expiry, CVV)
 */
public class DebitCardStrategy implements PaymentStrategy {
    // Simple id generator for payments created by this strategy, thread-safe atomic (start at 2000)
    private static final AtomicInteger idGen = new AtomicInteger(2000);
    ...
    public boolean validate(PaymentMethod method) {
        ...
        String expiry = method.getDetail("expiry");
        String cvv = method.getDetail("cvv");

        // Card number must be exactly 16 digits
        if (!cardNumber.matches("\\d{16}")) {
            System.out.println("[DebitCard] Invalid card number (must be 16 digits).");
            return false;
        }
        // CVV must be 3 or 4 digits
        if (!cvv.matches("\\d{3,4}")) {
            System.out.println("[DebitCard] Invalid CVV (must be 3-4 digits).");
            return false;
        }
        ...
        try {
            ...
            if (exp.isBefore(YearMonth.now())) {
                System.out.println("[DebitCard] Card is expired.");
                return false;
            }
        } catch (Exception e) {
            System.out.println("[DebitCard] Invalid expiry date format. Use MM/YY.");
            return false;
        }
    ...
    public Payment process(double amount, PaymentMethod method, int bookingId) {
        System.out.println("[DebitCard] Simulating payment processing...");
        
        // Pretend to wait as if talking to a card‑processor (this just makes it cooler)
        simulateDelay();
        
        // Create a fake transaction id
        String txId = "DC-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
        
        // Build and mark the payment as successful
        Payment payment = new Payment(idGen.getAndIncrement(), amount, txId, "DEBIT_CARD", bookingId);
        payment.markSuccess();
        
        System.out.println("[DebitCard] Payment successful. Transaction ID: " + txId);
        return payment;
    }
